    ...
    <section data-aos="fade-up" data-aos-delay="100" class="bg-[#F8F9FC] py-16 md:py-20">
        <div class="max-w-[85%] mx-auto">
            <div class="text-center mb-12 md:mb-14">
                <span class="text-[#4761FF] font-semibold text-sm tracking-widest uppercase">Insights</span>
                        ...
                        <article class="group bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgba(28,32,53,0.06)] overflow-hidden flex flex-col h-full transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_16px_40px_rgba(71,97,255,0.14)] hover:border-[#4761FF]/20">
                            ...
                            <div class="p-6 flex flex-col flex-1">
                                <div class="flex flex-wrap items-center gap-2 mb-3">
                                    @if ($post->published_at)
                                        <time datetime="{{ $post->published_at->toDateString() }}"
                                            ...
                                            <i class="fa-regular fa-calendar text-[#4761FF]"></i>
                                            {{ $post->published_at->format('M d, Y') }}
                                        </time>
                                    @endif

                                    @forelse ($post->categories as $category)
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full bg-[#EEF1FF] text-[#4761FF] text-xs font-semibold">
                                            {{ $category->name }}
                                        </span>
                                    @empty
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full bg-[#EEF1FF] text-[#4761FF] text-xs font-semibold">
                                ...
                                <h3 class="text-lg md:text-xl font-semibold text-[#1C2035] mb-3 leading-snug line-clamp-2">
                                    <a href="{{ route('blog.show', $post->slug) }}" class="hover:text-[#4761FF] transition">
                                        {{ $post->title }}
                                    </a>
                                </h3>
                                ...
                                <div class="pt-4 mt-auto border-t border-slate-100">
                                    <a href="{{ route('blog.show', $post->slug) }}"
                                       class="inline-flex items-center gap-2 text-[#4761FF] font-semibold text-sm group/link">
                                        Read More
